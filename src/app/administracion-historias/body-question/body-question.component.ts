...
export class BodyQuestionComponent implements AfterViewInit {
  ...
  convertirJsonACadena() {
    // Convierte el JSON a cadena y luego codifica en base64
    const jsonString = JSON.stringify(this.questions);
    this.jsonConvertido = btoa(jsonString);
  }

  desconvertirCadenaAJson() {
    // Decodifica la cadena base64 y luego parsea a JSON
    const decodedString = atob(this.jsonConvertido);
    this.jsonDesconvertido = JSON.parse(decodedString);
  }
